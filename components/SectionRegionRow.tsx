...
function MovieCardSmall({ movie }: { movie: Movie }) {
  ...
  return (
    <Link href={url} className="group block shrink-0 w-[100px] sm:w-[120px] md:w-[140px]" title={movie.name}>
      ...
      {movie.origin_name && (
        <p className="mt-1 text-[10px] text-white/50 truncate px-0.5">{movie.origin_name}</p>
      )}
    </Link>
  );
}
...
export default function SectionRegionRow({
  ...
  return (
    <section className="mb-10 relative overflow-hidden rounded-2xl border border-white/10 p-4 sm:p-5" style={{ backgroundColor: colors.bg }}>
      {content}
    </section>
  );
}
