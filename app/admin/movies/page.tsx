...
export default async function AdminMoviesPage({
  ...
  try {
    const isVisibleFilter = filter === "visible" ? true : filter === "hidden" ? false : undefined;
    moviesData = await getAllMovies({ page: currentPage, limit: 50, isVisible: isVisibleFilter, search });
  } catch (e) {
    errorMessage = e instanceof Error ? e.message : String(e);
    moviesData = { movies: [], total: 0, page: 1, limit: 50, totalPages: 0 };
  }
  ...
  return (
    <div>
      ...
      <VisibilityTabs currentFilter={filter} />
      <TelegramTestButton />
      <SearchBar />
      <div className="mt-6">
        {errorMessage && (
        ...
        <p className="text-white/60 text-sm mb-4">
          {moviesData.total === 0
            ? filter === "hidden"
              ? "Không có phim nào đang bị ẩn"
              : filter === "visible"
              ? "Không có phim nào đang hiện"
              : "Chưa có phim nào. Nhấn nút 'Đồng bộ phim' để tải phim từ API."
            : filter === "hidden"
            ? `${moviesData.total} phim đang bị ẩn`
            : filter === "visible"
            ? `${moviesData.total} phim đang hiện`
