import { getActorBySlug, getCategories, getRegions, getYears } from "@/lib/api";
...
export default async function ActorPage({
  ...
  let person: Awaited<ReturnType<typeof getActorBySlug>>["person"] | null = null;
  let data: Awaited<ReturnType<typeof getActorBySlug>>["data"] | null = null;
  ...
  const sectionName = `Diễn viên ${person.name}`;
...
export async function generateMetadata({
  ...
  const { slug } = await params;
  try {
    const res = await getActorBySlug(slug?.[0] ?? "", 1);
    return { title: `${res.person.name} | Dora Movies` };
  } catch {
    return { title: "Diễn viên | Dora Movies" };
  }
}
