import { getCatalogBySlug, getCategories, getRegions, getYears } from "@/lib/api";
...
export default async function CatalogSlugPage({
  ...
  let catalog: Awaited<ReturnType<typeof getCatalogBySlug>>["catalog"] | null = null;
  let data: Awaited<ReturnType<typeof getCatalogBySlug>>["data"] | null = null;
  ...
  try {
    const [catRes, regRes, yearsRes, slugRes] = await Promise.all([
      ...
      getRegions(),
      getYears(),
      getCatalogBySlug(slugStr, page),
    ]);
    categories = catRes;
    regionsList = regRes;
    yearsList = yearsRes;
    catalog = slugRes.catalog;
    data = slugRes.data;
  } catch {
    notFound();
  }
  ...
  const sectionName = `Danh sách ${catalog.name}`;
...
export async function generateMetadata({
  ...
  const { slug } = await params;
  try {
    const res = await getCatalogBySlug(slug?.[0] ?? "", 1);
    const title = res.catalog.seo_title || res.catalog.name;
    return { title: `${title} | Dora Movies` };
  } catch {
    return { title: "Danh sách | Dora Movies" };
  }
}
