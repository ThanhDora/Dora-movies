...
function ToggleSwitch({
  ...
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 shrink-0 rounded-full transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2 focus-visible:ring-offset-[#0a0d0e] cursor-pointer touch-manipulation ${
        checked ? "bg-[#ff2a14]" : "bg-white/25"
      }`}
    >
      <span
    ...
    <main className="w-full max-w-[1600px] mx-auto px-3 sm:px-4 py-4 sm:py-6">
      ...
      <div>
        <div className="min-w-0">
          ...
          <div
            className={
              cinemaMode
                ...
                : "w-full pb-[56.25%] relative h-0 bg-black rounded-lg overflow-hidden"
            }
          >
            {cinemaMode && (
              <div className="flex items-center justify-between px-4 py-2 bg-black/80 shrink-0">
                <span className="text-white font-medium truncate">{currentMovie.name} - Tập {currentEpisode.name}</span>
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 mt-3 py-2 border-b border-white/10">
            <button type="button" onClick={toggleFavorite} className="flex items-center gap-1.5 min-h-[40px] px-3 py-2 rounded-lg text-white/90 hover:bg-white/10 transition-colors" title="Yêu thích">
              <HeartIcon filled={favorite} />
              <span className="text-sm font-medium">Yêu thích</span>
            </button>
            <button type="button" onClick={() => showToast("Tính năng đang phát triển")} className="flex items-center gap-1.5 min-h-[40px] px-3 py-2 rounded-lg text-white/90 hover:bg-white/10 transition-colors" title="Thêm vào danh sách">
              <PlusIcon />
              <span className="text-sm font-medium">Thêm vào</span>
            </button>
            <div className="flex items-center gap-2 min-h-[40px] px-3 py-2 rounded-lg text-white/90 hover:bg-white/10">
              <ToggleSwitch checked={autoNext} onChange={setAutoNext} ariaLabel="Chuyển tập" />
              <span className="text-sm font-medium">Chuyển tập</span>
            </div>
            <div className="flex items-center gap-2 min-h-[40px] px-3 py-2 rounded-lg text-white/90 hover:bg-white/10">
              <ToggleSwitch checked={skipIntro} onChange={setSkipIntro} ariaLabel="Bỏ qua giới thiệu" />
              <span className="text-sm font-medium">Bỏ qua giới thiệu</span>
            </div>
            <div className="flex items-center gap-2 min-h-[40px] px-3 py-2 rounded-lg text-white/90 hover:bg-white/10">
              <ToggleSwitch checked={cinemaMode} onChange={setCinemaMode} ariaLabel="Rạp phim" />
              <span className="text-sm font-medium">Rạp phim</span>
            </div>
            {autoNext && nextEpisodePath && (
              <Link
                href={nextEpisodePath}
                className="flex items-center gap-1.5 min-h-[40px] px-3 py-2 rounded-lg bg-[#ff2a14] text-white text-sm font-medium hover:bg-[#e02512] transition-colors shrink-0"
              >
                <PlayIconSmall />
                Tập tiếp theo
              </Link>
            )}
            <button type="button" onClick={handleShare} className="flex items-center gap-1.5 min-h-[40px] px-3 py-2 rounded-lg text-white/90 hover:bg-white/10 transition-colors" title="Chia sẻ">
              <ShareIcon />
              <span className="text-sm font-medium">Chia sẻ</span>
            </button>
            <button type="button" onClick={scrollToReport} className="flex items-center gap-1.5 min-h-[40px] px-3 py-2 rounded-lg text-white/90 hover:bg-white/10 transition-colors" title="Báo lỗi">
              <FlagIcon />
          ...
          <div className="mt-4 flex flex-wrap items-center gap-3">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 min-h-[40px] px-4 py-2 rounded-lg bg-[#25252b] text-white font-medium text-sm"
              >
                {activeServer}
                <svg className={`w-4 h-4 transition-transform ${seasonOpen ? "rotate-180" : ""}`} fill="currentColor" viewBox="0 0 12 12">
                  <path d="M6 8L1 3h10z" />
                </svg>
              </button>
              {seasonOpen && (
                <>
                  ...
                  <div className="absolute left-0 top-full mt-1 py-2 bg-[#25252b] rounded-lg shadow-xl border border-white/10 z-20 min-w-[160px] max-h-[240px] overflow-auto">
                    {serverNames.map((s) => (
                      <button
                        key={s}
                        type="button"
            ...
            <button type="button" className="flex items-center gap-2 min-h-[40px] px-4 py-2 rounded-lg bg-[#25252b] text-white font-medium text-sm" title="Phụ đề">
              PHỤ ĐỀ
            </button>
              ...
              {episodeNamesSorted.map((name) => {
                ...
                return (
                  <Link
                    ...
                    className={`min-h-[40px] inline-flex items-center gap-1.5 px-3 py-2 rounded-lg text-sm font-medium transition-colors touch-manipulation ${isActive ? "bg-amber-500/90 text-black" : "bg-[#25252b] text-white hover:bg-[#2a2a32]"}`}
                  >
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-6">
            <div className="shrink-0 flex justify-center sm:block">
              <div className="relative w-full max-w-[180px] aspect-2/3 rounded-lg overflow-hidden bg-[#232328] mx-auto sm:mx-0">
                {(currentMovie.poster_url || currentMovie.thumb_url) ? (
                  <Image src={currentMovie.poster_url || currentMovie.thumb_url || ""} alt="" width={180} height={270} className="object-cover w-full h-full" unoptimized={(currentMovie.poster_url || currentMovie.thumb_url || "").startsWith("http")} />
                ) : (
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-xl font-bold text-white">{currentMovie.name}</h1>
              {currentMovie.origin_name ? <p className="text-white/70 text-sm mt-0.5">{currentMovie.origin_name}</p> : null}
              <div className="flex flex-wrap gap-2 mt-2">
                {currentMovie.quality ? <span className="px-2.5 py-1 rounded bg-white/15 text-white text-xs">{currentMovie.quality}</span> : null}
                {currentMovie.episode_time ? <span className="px-2.5 py-1 rounded bg-white/15 text-white text-xs">{currentMovie.episode_time}/tập</span> : null}
                {currentMovie.language ? <span className="px-2.5 py-1 rounded bg-white/15 text-white text-xs">{currentMovie.language}</span> : null}
              </div>
              <div className="flex flex-wrap gap-2 mt-2">
                {(currentMovie.categories || []).map((c) => (
                  <Link key={c.id} href={c.url || `/the-loai/${c.slug}`} className="px-3 py-1.5 rounded-lg bg-black/60 text-white text-sm hover:bg-black/80 transition-colors">
                    {c.name}
                  </Link>
                ))}
              </div>
              {currentMovie.content ? (
                <p className="text-white/85 text-sm leading-relaxed mt-3 line-clamp-6">{stripHtml(currentMovie.content)}</p>
              ) : null}
